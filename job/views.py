...
def add_job(request):
    ...
    if request.method == 'POST':
        ...
        recruiter = Recruiter.objects.get(user=user)
        try:
            Job.objects.create(recruiter=recruiter,title=jt,start_date=sd,end_date=ed,salary=sal,image=l,description=des,experience=exp,location=loc,skills=skills,creationdate=date.today())
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
...
def latest_jobs(request):
    data = Job.objects.all().order_by('-start_date')[:6]
    d = {'data':data}
...
def applyforjob(request,pid):
    ...
    if job.end_date < date1:
        ...
        if request.method == 'POST':
            ...
            Apply.objects.create(resume=r,apply_date=date.today(),student=student,job=job)
            error="done"
